...
const ServiceCard = ({ card }) => {
  const { title, services, description, number, classes } = card;
  const { ref } = useInView({
    triggerOnce: true,
    threshold: 0.6,
  });
  ...
  return (
    <div ref={ref} key={number} className="px-[6vw] md:px-[3vw] pb-[9.5vw] last:pb-[13vw]">
      <h4 className="text-[3.7vw] md:text-[7vw] md:mt-[3vw] font-light text-white">{title}</h4>
        ...
        <div className="flex-1 md:mb-[2vw]">
          <div className="flex flex-wrap space-y-[2vw] md:space-y-[1.5vw]">
            {services.map((service, i) => (
              <ul key={i} className="flex items-center space-x-[5vw] text-[1.7vw] md:text-[3vw] font-semibold">
                {service.map((s) => (
